...
import { Col, Row, Tab, Alert, InputGroup, Form, Button } from 'react-bootstrap';
...
import { faTruck, faInfoCircle, faBox, faMapMarkerAlt, faFileAlt } from '@fortawesome/free-solid-svg-icons';
...
const CreateNewLoad: React.FC = () => {
  ...
  const handleNext = () => setSelectedStep(prev => Math.min(prev + 1, wizardNavItems.length));
  const handleFinalSubmit = async () => {
    try {
      const result = await createLoad(form.formData);
      setAlert({ show: true, message: 'Load data saved successfully', variant: 'success' });
    } catch (error) {
      setAlert({ show: true, message: 'Failed to save load data', variant: 'danger' });
    }
  };
  ...
  return (
    <div className="mb-9">
      <PageBreadcrumb items={[{ label: 'Home', url: '/' }, { label: 'Operations' }, { label: 'Create New Load' }]} />
      ...
      <WizardFormProvider
        ...
        setFormData={form.setFormData as React.Dispatch<unknown>}  // Type casting to match expected type
        validation={form.validation}
        startOver={() => setSelectedStep(1)}
        getCanNextPage={selectedStep < wizardNavItems.length}
        getCanPreviousPage={selectedStep > 1}
        openDeniedModal={false}
        setOpenDeniedModal={() => {}}  // Placeholder function, replace if actual logic needed
        setValue={() => {}}  // Placeholder for form value handling
        onChange={() => {}}  // Placeholder function, replace with actual change handler if necessary
        onSubmit={() => {}}  // Placeholder function, replace with form submit handler
      >
        <Row className="gx-0 gx-xl-5 theme-wizard">
          ...
          <Col xl={8}>
            <Tab.Content>
              {selectedStep === 1 && (
                <Tab.Pane eventKey={1}>
                  <InputGroup className="mb-3">
                    ...
                    />
                    <Button variant="link" onClick={handleShowNewCustomerModal}>Add New Customer</Button>
                  </InputGroup>
                  {searchResults.length ? (
                    <ul className="list-group">
                      {searchResults.map((customer: any, index) => (
              ...
              {selectedStep === 2 && (
                <Tab.Pane eventKey={2}>
                  <PickupDropoffDetails formData={form.formData} onChange={form.onChange} validation={form.validation || false} />
                </Tab.Pane>
              )}

              {selectedStep === 3 && (
                <Tab.Pane eventKey={3}>
                  <TrailerSpecifications formData={form.formData} onChange={form.onChange} validation={form.validation || false} />
                </Tab.Pane>
              )}

              {selectedStep === 4 && (
                <Tab.Pane eventKey={4}>
                  <ShipmentDetails formData={form.formData} onChange={form.onChange} validation={form.validation || false} />
                </Tab.Pane>
              )}

              {selectedStep === 5 && (
                <Tab.Pane eventKey={5}>
                  <AdditionalInformation formData={form.formData} onChange={form.onChange} validation={form.validation || false} />
                </Tab.Pane>
              )}
            </Tab.Content>
      ...
      <NewCustomerModal show={showNewCustomerModal} onHide={handleCloseNewCustomerModal} />
